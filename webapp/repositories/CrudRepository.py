...
class CrudRepository:
    ...
    def get_by(self,to_append, **kwargs):
        """
...
def formChoices(model, attribute=None, context=None):

    len_choices = range(len(model.query.all()))
    if type(model.query.all()) != type([]): 
        len_choices=1

    u=0
    choices=[]
    if context == None and attribute ==None:

        choices.append([((j+1,  str(model.query.all()[j]))) for j in len_choices])

    elif context == None and type(attribute)!= type([]) :

        choices.append([((j+u+1,str(getattr(model.query.all()[j], attribute)))) for j in len_choices])
        u+=1
    elif context == None and type(attribute)== type([]) :
        for a in attribute:
            choices.append([((j+u+1,str(getattr(model.query.all()[j], a)))) for j in len_choices])
            u+=1
    elif len_choices != 1 and context != None and type(attribute)== type([]):
        for a, c in zip(attribute,context):
        
            choices.append([((j+u+1, c + str(getattr(model.query.all()[j], a)))) for j in len_choices])
            u+=1
    elif len_choices==1 and context != None and type(attribute)== type([]):
        for a, c in zip(attribute,context):
        
            choices.append(((u+1, c + str(getattr(model, a)))))
            u+=1
